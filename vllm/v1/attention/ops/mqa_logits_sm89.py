...
Both paths take the cached e4m3 bytes as they are. Prefill dots them on sm_89's
native FP8 tensor cores (mma.sync.m16n8k32 with .e4m3 operands); paged decode is
bandwidth bound, so it widens to BF16 and keeps the exact dot product. The cache
never holds 0x7F/0xFF -- the compressor clamps to the e4m3 max of 448 -- so the
widening needs no special cases.
...
# Keys per program in prefill: one q tile amortised over KV_GROUP key blocks.
_PREFILL_KV_GROUP = 4
...
def _fp8_paged_mqa_logits_kernel(
    ...
    q = tl.load(q_base + q_offs, mask=q_mask, other=0.0).to(tl.bfloat16)
...
def fp8_paged_mqa_logits_triton(
    ...
    # A block holds its block_size * head_dim value bytes first, then the
    # per-token float32 scales.
    kv_values = (
        kv_flat[:, :k_end]
